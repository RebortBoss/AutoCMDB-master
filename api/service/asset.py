...
class HandleBasic:
    ...
    def process(server_obj,server_info,user_obj):
        ...
        try:
            ...
            response.message = str(e)
            models.ErrorLog.objects.create(asset_obj=server_obj.asset,title='basic-run',content=traceback.format_exc())
...
class HandleNic:
    ...
    @staticmethod
    def _del_nic(del_list,nic_obj,server_obj,user_obj):
        for item in nic_obj:
    ...
    @staticmethod
    def _update_nic(update_nic,nic_obj,client_nic_dict,server_obj,user_obj):
